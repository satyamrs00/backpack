                ...
                <div className="row">
                    ...
                    <div style={{ fontFamily: 'serif' }} className={`col-${window.screen.width > 900 ? 6 : 12}`}>
                        <div className='row' style={{ padding: '1rem 1.5rem .1rem 1.5rem' }}>
                            ...
                            <img className='col-2' src={data.isAvailable ? available : notavailable} alt={data.isAvailable ? 'available' : 'not available'} style={{ width: 'calc(3.5rem + .5vw)', height: "calc(2rem + .5vw)" }} />
                        </div>
                        ...
                        <div className="footer d-flex justify-content-end" style={{ backgroundColor: 'rgb(187 187 187)', padding: '.5rem 1rem' }}>
                            {data.isAvailable && <button className='btn shadow-sm' style={{ backgroundColor: "orange", fontWeight: '600', color: '#404040' }} onClick={handleClick}>Request Book</button>}
                            {!data.isAvailable && <span className='text-danger p-1'>Currently unavailable</span>}
                        </div>
                    </div>
                </div>
            </div>
